...
export function MistakePracticeFinishDialog({ practice, onContinueWrong, onExit, onClose }: MistakePracticeFinishDialogProps) {
  ...
  return (
    <div className="modal-backdrop review-finish-backdrop" onMouseDown={onClose}>
      <article className="daily-review-finish-dialog" role="dialog" aria-modal="true" aria-label="完成错题刷题" onMouseDown={(event) => event.stopPropagation()}>
        <header className="daily-review-finish-header">
          <span className="daily-review-finish-icon">
            <AlertTriangle size={24} strokeWidth={2.4} />
          </span>
          <div>
            <span className="eyebrow">错题刷题</span>
            <h2>本轮错题怎么处理？</h2>
          </div>
        ...
        <p className="daily-review-finish-copy">
          可以直接退出错题刷题；如果本轮又做错了题，也可以只把这些题继续生成下一轮错题刷题。
        </p>
